#!/usr/bin/python
#
# A utility to convert a VLBI 'VEX' file into commands
# that call 'm5fb' (part of DiFX software correlator tools) 
# which generates filterbank data from raw VLBI 2-bit
# wideband time domain sample recordings.
#
import os
import sys
import re
...
def extract_info(filename):
  os.system('source /share/apps/difx/setup.bash;vexpeek '+filename+' -v |less > vexstat.txt')
...
def getTele(FileList):
  ...
  if Letters=='.KY': NumID='21'
  if Letters=='.KT': NumID='22'
...
def main(): 
  opts,args = getopt.getopt(sys.argv[1:],"hx:l:s:t:m:o:p:n:f:d:")
  ...
  if help:
    print ''
    print 'filterbankprep.py [-h] -x VEX -l FILELIST -s SOURCE -t TIMEUSEC -m MODE -n NCHAN -f TopBandEdge'
    exit(0)
